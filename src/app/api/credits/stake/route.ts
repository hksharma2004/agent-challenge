...
function getTier(stakedAmount: number): StakingTier {
  if (stakedAmount >= STAKING_TIERS[StakingTier.GOLD]) {
    return StakingTier.GOLD;
  }
  if (stakedAmount >= STAKING_TIERS[StakingTier.SILVER]) {
    return StakingTier.SILVER;
  }
  if (stakedAmount >= STAKING_TIERS[StakingTier.BRONZE]) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { amount } = stakeSchema.parse(json);
    ...
    if (error) {
      console.error('Error staking credits:', error);
      return new NextResponse(
        JSON.stringify({ error: 'Failed to stake credits', details: error.message }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
        }
    ...
    console.error('Unexpected error staking credits:', error);
